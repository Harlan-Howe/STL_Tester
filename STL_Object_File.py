...
class STL_Object:
    ...
    def apply_transform(self,A: np.ndarray) -> np.ndarray:
        ...
        array of points that you want to vector multiply by a (4 x 4) matrix, A: result = A • p for each of these points,
        but if you try multiplying matrices with shape (4, 4) • (N, 3, 4), you get an error. So I've employed a trick to
        turn the list into a (4, 3N) matrix, which we can multiply, and then convert this back into a (N, 3, 4) matrix.
        """
        ...
        # renormalize vector - divide the resulting (wx, wy, wz, w) matrix by w to get (x, y, z ,1)
    ...
    def convert_to_2d(self, A: np.ndarray = None, d1: float = 400, d2: float= 400) -> np.ndarray :
        """
        A is the (4 x 4) transform to apply to this 3-d shape before rendering.
        d1 is the distance from the eye to the screen
        d2 is the distance from the eye to the origin
        :param d1:
        :param d2:
    ...
    def find_normals(self,transform: np.ndarray = None) -> np.ndarray:
        ...
        #------------------------------------------------------------------------
        # TODO: (if you are trying to do shading or back-facing culling) calculate a list of normal vectors to each face. This is done by
        # finding the vector from point 0 - point 1 and then finding the vector from point 1 to point 2. The normal is
        # the cross product of these two vectors. You should research the cross product in numpy for this - it should
        # not be complicated in your code!

        first_sides: np.ndarray = faces[:, 1, 0:3] - faces[:, 0, 0:3] # I've done the first side - the result of this line is an
                                                          # (N x 3) matrix of (dx, dy, dz).

        #------------------------------------------------------------------------

    def normalize(self,list: np.ndarray) -> np.ndarray:
        """
        ...
        :param list: a (N x 3) list of 3-vectors
    ...
    def draw_self(self,
        ...
        # ----------------------------------------------------------------------------
        # TODO: PHASE 1: (rendering) get a list of 2-d points for these faces via another method you have drawn already. Then draw
        # the faces.

        # You might do this via three of cv.line() or one of cv.polylines()


        # TODO: PHASE 2: (backface culling) calculate the normal to each face. Use the dot product with a line in the z direction to
        # determine whether to draw each triangle or not - NOTE: this means altering what was written in phase 1. You
        # might wish to copy that code here and comment out the original before you make your changes.


        # TODO: PHASE 3: (shading) Use the directions of the normals, the direction from the midpoints to the eye and/or the
        # direction of a light source (your choice). Find the relative angles between these vectors via the dot product
        # (or just use the dot product, itself) to choose a color (or shade of grey) for this face. You will want to use
        # cv.fillConvexPoly() for this. Filling the polys in grey and then drawing the polylines in black on top can be
        # pretty cool.
